...
<Latex centered={true} f="\frac{\delta y}{\delta t} = f(y,\ t);\ a \le t \le b;\ y(a) = \alpha" />
...
<Latex centered={true} f="y(t_{i+1}) = y(t_i) + (t_{i+1}-t_{i}) \cdot y'(t_i) + \frac{(t_{i+1}-t_{i})^{2}}{2}y''(\xi_{i})" />
...
<Latex centered={true} f="y_{i+1} = y_i + h \cdot f(y_i,\ t_i)" />
...
<Latex centered={true} f="\frac{\delta y}{\delta t} = y - t^{2} + 1;\ 0 \le t \le 2;\ y(0) = 0.5 ;\ h = 0.2" />
...
<Latex centered={true} f="t_0 = 0.0, t_1 = 0.2, t_2 = 0.4, t_3 = 0.6, t_4 = 0.8, " />
<Latex centered={true} f="t_5 = 1.0, t_6 = 1.2, t_7 = 1.4, t_8 = 1.6, t_9 = 1.8" />
...
<Latex centered={true} f="\omega_{0} = 0.5" />

<Latex centered={true} f="\omega_{i + 1} = \omega_{i} + h \cdot (\omega_{i} - t_{i}^{2} + 1)" />

So <Latex f="\omega_{0 + 1} = \omega_{1}"/> for the first iteration is going to be:

<Latex centered={true} f="\omega_{1} = 0.5 + 0.2 \cdot (0.5 - 0^{2} + 1) = 0.8" />

And the rest of iterations are:

<Latex centered={true} f="\omega_{2} = 0.8 + 0.2 \cdot (0.8 - 0.2^{2} + 1) = 1.152" />

<Latex centered={true} f="\omega_{3} = 1.152 + 0.2 \cdot (1.152 - 0.4^{2} + 1 = 1.5504" />

<Latex centered={true} f="\omega_{4} = 1.5504 + 0.2 \cdot (1.5504 - 0.6^{2} + 1) = 1.9884" />

<Latex centered={true} f="\omega_{5} = 1.9884 + 0.2 \cdot (1.9884 - 0.8^{2} + 1) = 2.4580" />

<Latex centered={true} f="\omega_{6} = 2.4580 + 0.2 \cdot (2.4580 - 1.0^{2} + 1) = 2.9498" />

<Latex centered={true} f="\omega_{7} = 2.9498 + 0.2 \cdot (2.9498 - 1.2^{2} + 1) = 3.4517" />

<Latex centered={true} f="\omega_{8} = 3.4517 + 0.2 \cdot (3.4517 - 1.4^{2} + 1) = 3.9501" />

<Latex centered={true} f="\omega_{9} = 3.9501 + 0.2 \cdot (3.9501 - 1.6^{2} + 1) = 4.4281" />

<Latex centered={true} f="\omega_{10} = 4.4281 + 0.2 \cdot (4.4281 - 1.8^{2} + 1) = 4.8657" />

So the approximate value of <Latex f="y(2)"/> is <Latex f="\omega_{10} = 4.8657" />. For comparison, the exact solution of the ODE is:

<Latex centered={true} f="y(t) = t^{2} + 2t + 1 - 0.5e^{t}" />

So the exact value of <Latex f="y(2)"/> is:

<Latex centered={true} f="y(2) = 2^{2} + 2 \cdot 2 + 1 - 0.5e^{2} = 5.3054" />

So the error of the Euler method is:

<Latex centered={true} f="\text{Error} = |4.8657 - 5.3054| = 0.4397" />
...
<CodeBlock c={`import numpy as np
